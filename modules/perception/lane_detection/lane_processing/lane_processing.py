...
import cv2
import numpy as np
from image_processing.image_processing import generate_images, test_image, load_test_images
import matplotlib.pyplot as plt

from video_processing.video_processing import read_video_to_frames
...
last_angles = []
imgs = read_video_to_frames("../video_processing/video_test.mp4")
for img in imgs:
    results = generate_images(img)
    offset = offset_from_center(results['warped'])
    angles = steering_angle(results["warped"])
    last_angles.append(angles)
    if len(last_angles) > 30:
        last_angles.pop(0)
    print("Angle ", np.mean(np.asarray(last_angles)), "Offset: ", offset)
